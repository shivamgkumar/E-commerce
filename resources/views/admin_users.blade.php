  ...
  <main class="main">
        ...
        <header class="d-flex justify-content-end mt-3">
            <button class="add_btn bg-blue-1-hover text-white btn rounded" data-bs-toggle="modal"
                data-bs-target="#user-modal" title="Add New Client">+ Add User</button>`
        </header>
            ...
            <table class="table table-bordered table-striped table-hover">
                <thead>
                    ...
                    <tr>
                        ...
                        <th>Role</th>
                        <th>Status</th>
                        <th  class="no-filter" id="editdeleteTh">Actions</th>
                    </tr>
                 </thead>
                <tbody class="table-group-divider">
                    ...
                    <tr class="tr_ container-fluid">
                        <td>{{ $i++ }}</td>
                        <td id="category">{{ $users['name']  ?? 'N/A'}}</td>
                        <td>{{ $users['email']  ?? 'N/A'}}</td>
                        <td>{{ $users['role']  ?? 'N/A'}}</td>
                        <td>{{ $users['status']  ?? 'N/A'}}</td>

                    <td class="editdeleteTd">
                 <div class="d-flex justify-content-center gap-1 ">
               <button class="editBtn btn p-2" data-category-for-edit="{{ $users }}" data-cat_id_for_edit="{{ $users->id }}" id="edtBtn" data-bs-toggle="modal" data-bs-target="#user-modal" title="Edit client Info">
            <i class="fa-solid fa-pen"></i>
        </button>
                                <button class="btn p-2 deleteBtn" data-id="{{ $users['id'] }}">
    ...
    <div class="modal fade" id="user-modal" tabIndex="-1" data-bs-backdrop="static">
        <div class=" modal-dialog  modal-dialog-scrollable ">
            ...
            <form method="POST" id="userForm" class="d-flex flex-column gap-3" enctype="multipart/form-data">
            @csrf

            <div>
                <label for="name">Name: </label>
                <input type="text" name="name" class="form-control" id="user_name" placeholder="Name" minlength="3" maxlength="50" required />
            </div>

               <div>
                <label for="user_email">Email: </label>
                <input type="email" name="email" class="form-control" id="user_email"  placeholder="Email" maxlength="50" required />
            </div>

               <div>
                <label for="user_password">Password: </label>
                <input type="password" name="password" class="form-control" id="user_password"  placeholder="Password" minlength="8" maxlength="50" required />
            </div>
...
<script>
    ...
    const userRole = "{{ Auth::user()->role }}";
      if (userRole !== 'admin') {
        $('.editdeleteTd').hide(); 
        $('#editdeleteTh').css('display', 'none');
        $('.editdeleteTfootTh').css('display', 'none');
      } else {
        $('.editdeleteTd').show(); 
        $('#editdeleteTh').css('display', 'table-cell');
        $('.editdeleteTfootTh').css('display', 'table-cell');
    }
    ...
    $('#userForm').off('submit').on('submit', function (e) {
       ...
       $('#saveBtn').data('submitting', true);
        ...
        let url = location+'admin/add_user';
...
//         var url = categoryId
//             ? 'http://127.0.0.1:8000/admin/updateCategory'
//             : 'http://127.0.0.1:8000/admin/addCategory';
...
<script>
    ...
    $('.deleteBtn').click(function() {
        ...
        const url=window.location.origin+`/admin/delete_user/${delete_id}`;
